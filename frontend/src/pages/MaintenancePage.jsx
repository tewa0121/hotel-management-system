...
const MaintenancePage = () => {
  ...
  const getNextAction = (status) => {
    const actions = {
      open: { next: 'assigned', label: 'Assign', color: 'bg-blue-600 hover:bg-blue-700' },
      assigned: { next: 'in_progress', label: 'Start', color: 'bg-yellow-600 hover:bg-yellow-700' },
      in_progress: { next: 'completed', label: 'Complete', color: 'bg-green-600 hover:bg-green-700' },
      completed: null,
      cancelled: null
    };
  ...
  const filteredRequests = requests.filter(req => {
    const matchSearch = req.room_number?.includes(search) || 
                        ...
                        req.category?.includes(search.toLowerCase());
    return matchSearch;
  });
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredRequests.map((request) => {
            const action = getNextAction(request.status);
            return (
              <div key={request.id} className="card hover:shadow-lg transition-shadow border-l-4 border-l-transparent hover:border-l-primary-500">
